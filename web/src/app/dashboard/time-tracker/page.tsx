...
import { GET_ACTIVE_TIME_ENTRY, CHECK_IN, CHECK_OUT, START_TIME_ENTRY, STOP_TIME_ENTRY } from '@/lib/graphql/queries';
import { useState, useEffect } from 'react';
import { ClockIcon, PlayIcon, StopIcon } from '@heroicons/react/24/outline';

export default function TimeTrackerPage() {
    const { data: activeEntryData, refetch } = useQuery(GET_ACTIVE_TIME_ENTRY);
    const [checkIn] = useMutation(CHECK_IN, { onCompleted: () => refetch() });
    const [checkOut] = useMutation(CHECK_OUT, { onCompleted: () => refetch() });
    const [startTimer] = useMutation(START_TIME_ENTRY, { onCompleted: () => refetch() });
    const [stopTimer] = useMutation(STOP_TIME_ENTRY, { onCompleted: () => refetch() });
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="px-3 py-1 text-sm rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400">
                                    Pending
                                </span>
                            </div>
                        </div>
